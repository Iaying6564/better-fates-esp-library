...
function Utils.SmoothScroll(content, SmoothingFactor) -- by Elttob
	...
	local function syncProperty(prop)
		Connect(content:GetPropertyChangedSignal(prop), function()
			if prop == "ZIndex" then
	...
	local smoothConnection = Connect(RunService.RenderStepped, function()
	...
	Connect(content.AncestryChanged, function()
